...
import { TaskForceView } from '../components/TaskForceView'
import { ScorePanel } from '../components/ScorePanel'
import { GameLog } from '../components/GameLog'
import { ActionPanel } from '../components/ActionPanel'
import { AirstrikePanel } from '../components/AirstrikePanel'
import { DestroyerResolvePanel } from '../components/DestroyerResolvePanel'
import type { ActionTarget, AirstrikeDeclaration } from '../types/game'
...
export default function GamePage() {
  ...
  const mySquadron = useMemo(
    () => destroyerSquadrons.find((s) => s.owner_id === user?.id),
    [destroyerSquadrons, user]
  )
  ...
  function handlePlayTarget(cardId: string, target: ActionTarget) {
    run(() => gameAction({ gameId: game!.id, type: 'play', cardId, target }))
  }

  if (game.status === 'lobby') {
    return <Lobby game={game} players={players} isHost={game.host_id === user.id} />
  }

  const myForce = taskForces[user.id]
  ...
  return (
    <div className="felt-table min-h-screen text-white">
      <div className="mx-auto grid max-w-6xl grid-cols-1 gap-4 px-4 py-6 lg:grid-cols-[1fr_280px]">
          ...
          {!mustResolveSquadron && !hasPendingDrawn && mode === 'airstrike' && myForce && (
            <AirstrikePanel
              ...
              onConfirm={(strikes: AirstrikeDeclaration[]) =>
                run(() => gameAction({ gameId: game.id, type: 'airstrike', strikes }))
              }
            />
          )}
          ...
          <div className="rounded-xl border border-white/15 bg-black/25 p-3">
            <div className="mb-2 flex items-center justify-between">
              ...
              {isMySpecialTurn && (
                <button
                  disabled={busy}
                  onClick={() => run(() => gameAction({ gameId: game.id, type: 'pass_special' }))}
              ...
              {isMyNormalTurn && !hasPendingDrawn && !mustResolveSquadron && (
                <div className="flex gap-2">
                  ...
                  {selectedCardId && (
                    <button
                      disabled={busy}
                      onClick={() => run(() => gameAction({ gameId: game.id, type: 'discard', cardId: selectedCardId }))}
                      className="rounded-md border border-white/20 px-3 py-1 text-xs text-white/80 hover:bg-white/10"
                    >
                      Discard selected
                    </button>
                  ...
                  <button
                    disabled={busy}
                    onClick={() => run(() => gameAction({ gameId: game.id, type: 'draw' }))}
                    className="rounded-md bg-amber-400 px-3 py-1 text-xs font-semibold text-black hover:bg-amber-300 disabled:opacity-40"
                  >
                    Draw
                  </button>
        ...
        <div className="space-y-4">
          <ScorePanel game={game} players={players} myUserId={user.id} />
